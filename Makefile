...
KERNEL_CONFIG_KEY := LOG=$(KERNEL_LOG_KEY) PERF_PROBE=$(KERNEL_PERF_PROBE_KEY) KERNEL_TRAP_DIAGNOSTICS=$(KERNEL_TRAP_DIAGNOSTICS_KEY)
...
$(KERNEL_BUILD_STAMP_RV): $(KERNEL_BUILD_DEPS) $(KERNEL_CONFIG_STAMP_RV) | $(STAMP_DIR)
	$(MAKE) -C os kernel ARCH=riscv64 KERNEL_TRAP_DIAGNOSTICS=$(KERNEL_TRAP_DIAGNOSTICS)
	touch $@

$(KERNEL_BUILD_STAMP_LA): $(KERNEL_BUILD_DEPS) $(KERNEL_CONFIG_STAMP_LA) | $(STAMP_DIR)
	$(MAKE) -C os kernel ARCH=loongarch64 KERNEL_TRAP_DIAGNOSTICS=$(KERNEL_TRAP_DIAGNOSTICS)
	touch $@
