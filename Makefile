...
KERNEL_CONFIG_KEY := BOARD=$(BOARD) LOG=$(KERNEL_LOG_KEY) PERF_PROBE=$(KERNEL_PERF_PROBE_KEY) KERNEL_TRAP_DIAGNOSTICS=$(KERNEL_TRAP_DIAGNOSTICS_KEY) COSMOS_MEMINFO=$(COSMOS_MEMINFO_KEY) SYSCALLS_COUNT=$(SYSCALLS_COUNT_KEY) IO_PERF_COUNTERS=$(IO_PERF_COUNTERS_KEY)
...
$(KERNEL_BUILD_STAMP_RV): $(KERNEL_BUILD_DEPS) $(KERNEL_CONFIG_STAMP_RV) | $(STAMP_DIR)
	$(MAKE) -C os kernel ARCH=riscv64 BOARD=$(BOARD) KERNEL_TRAP_DIAGNOSTICS=$(KERNEL_TRAP_DIAGNOSTICS) COSMOS_MEMINFO=$(COSMOS_MEMINFO) SYSCALLS_COUNT=$(SYSCALLS_COUNT) IO_PERF_COUNTERS=$(IO_PERF_COUNTERS)
	touch $@

$(KERNEL_BUILD_STAMP_LA): $(KERNEL_BUILD_DEPS) $(KERNEL_CONFIG_STAMP_LA) | $(STAMP_DIR)
	$(MAKE) -C os kernel ARCH=loongarch64 BOARD=$(BOARD) KERNEL_TRAP_DIAGNOSTICS=$(KERNEL_TRAP_DIAGNOSTICS) COSMOS_MEMINFO=$(COSMOS_MEMINFO) SYSCALLS_COUNT=$(SYSCALLS_COUNT) IO_PERF_COUNTERS=$(IO_PERF_COUNTERS)
	touch $@
