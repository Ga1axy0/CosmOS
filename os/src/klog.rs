...
impl KernelLogBuffer {
    ...
    fn read_into(&mut self, dst: &mut [u8]) -> usize {
        let to_read = dst.len().min(self.bytes.len());
    ...
    fn copy_recent_into(&self, skip: usize, dst: &mut [u8]) -> usize {
        let available = self.since_clear.min(self.bytes.len());
        if skip >= available {
            return 0;
        }
...
fn syslog_action_read_all(bufp: *mut u8, size: usize) -> isize {
    ...
    syscall_body!({
        ...
        let available = klog.since_clear.min(klog.bytes.len());
        ...
        for slice in user_bufs {
            ...
            copied += klog.copy_recent_into(available - target_len + copied, &mut slice[..chunk_len]);
        }
        Ok(copied as isize)
    })
}
...
fn syslog_action_read_clear(bufp: *mut u8, size: usize) -> isize {
    ...
    syscall_body!({
        ...
        for slice in user_bufs {
            ...
            let chunk_len = (target_len - copied).min(slice.len());
            copied +=
                klog.copy_recent_into(available - target_len + copied, &mut slice[..chunk_len]);
        }
...
fn syslog_action_size_unread() -> isize {
    KLOG_BUFFER.lock().bytes.len() as isize
}
