...
fn add_timer_with_tag_on_hart(
    ...
    let should_reprogram = {
        let mut timers = PER_CPU_TIMERS[target_hart].lock();
        let old_deadline = timers.peek().map(|timer| timer.expire_ns);
        timers.push(TimerCondVar {
            expire_ns,
            task,
            timer_tag,
        });
        old_deadline.map_or(true, |old_deadline| expire_ns < old_deadline)
    };
    if target_hart == normalize_hart(hartid()) && should_reprogram {
        program_next_trigger_for_hart_with_deadline(target_hart, get_time(), Some(expire_ns));
    }
}
...
fn program_next_trigger_for_hart(hart: usize, now_raw: usize) {
    program_next_trigger_for_hart_with_deadline(
        hart,
        now_raw,
        next_timer_deadline_ns_for_hart(hart),
    );
}

fn program_next_trigger_for_hart_with_deadline(
    hart: usize,
    now_raw: usize,
    timer_deadline_ns: Option<u64>,
) {
    let periodic_raw = *PER_CPU_NEXT_PERIODIC_TICK[hart].lock();
    let timer_raw = timer_deadline_ns.map(ns_to_raw_ticks_ceil);
